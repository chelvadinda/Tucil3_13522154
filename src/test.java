...
public class main {
    ...
    private static void loadDictionary() {
        try (BufferedReader reader = new BufferedReader(new FileReader(DICTIONARY_FILE_PATH))) {
            ...
            while ((word = reader.readLine()) != null) {
                dictionary.add(word.toLowerCase()); // Ubah kata menjadi lowercase untuk pencarian yang case-insensitive
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private static boolean isValidEnglishWord(String word) {
        return dictionary.contains(word.toLowerCase()); // Pencarian yang case-insensitive
    }
    ...
    public static void main(String[] args) {
        ...
        try (Scanner scanner = new Scanner(System.in)) {
            ...
            switch (algorithmChoice) {
                case 1:
                    UCS ucsSolver = new UCS(); // Membuat objek solver UCS
                    ucsSolver.solve(startWord, endWord); // Menyelesaikan masalah dengan algoritma UCS
                    break;
                case 2:
                    GreedyBestFirstSearch gbfsSolver = new GreedyBestFirstSearch(); // Membuat objek solver Greedy Best
                                                                                    // First Search
